...
const ExperienceForm = ({data = [], onChange}) => {
    ...
    const generateDesc = async(index)=>{
        ...
        const prompt = `Enhance this job description ${experience.description} for the position 
        of ${experience.position} at ${experience.company}`

        try{

            const {data} = await api.post('/api/ai/enhance-job-description' , {userContent:
                prompt} , {
                headers : {
                    Authorization : token
                }
            })
            updateExperience(index , 'description' , data.enhancedDescription)
    ...
    <div className='space-y-4'>
        <div>
            ...
            <div>
                ...
                <p className='text-sm text-gray-500 w-2xs'>Add your professional job experience</p>
            </div>
            <button 
        ...
        {data.length === 0 ? (
            <div className='text-center py-8 text-gray-500'>
                ...
                <p className='text-sm'>Click "Add Experience" to start adding your job history.</p> 
            </div>
        ) : (
            <div className='space-y-4'>
                {
                        ...
                        <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                            ...
                            <div className='space-y-2'>
                                <div className='flex items-center justify-between'>
                                    ...
                                    <button
                                    onClick={()=>generateDesc(index)}
                                    disabled={generatingIndex === index || !experience.position || 
                                        !experience.company
                                    }
                                    ...
                                    disabled:opacity-50 disabled:cursor-not-allowed'>
                                        ...
                                        {generatingIndex === index ? 'Generating' : 'AI Enhance'}
                                    </button>
                                </div>
