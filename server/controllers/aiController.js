...
// Fallback model to prevent server crash
  ...
  try {
    ...
    const response = await ai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an AI assistant that rewrites professional summaries 
          in exactly 2–3 short lines ATS friendly. 
          Keep it concise, professional, and impactful. 
          Do NOT add new skills or details not provided by the user. 
          Final output must be under 60 words. 
  ...
  try {
    ...
    const response = await ai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an AI assistant that rewrites job descriptions 
          in exactly 2–3 short lines. 
          Keep it concise, professional, and impactful. 
          Do NOT add new skills or details not provided by the user. 
          Final output must be under 60 words. 
...
const uploadResume = async (req, res) => {
  try {
    ...
    const systemPrompt =
      "You are a helpful AI assistant that extracts structured data from resumes.";

    const userPrompt = `Extract data from the following resume.
